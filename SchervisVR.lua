...
cam.HeadScale = settings.headscale
...
for i,v in pairs(char.Humanoid:GetAccessories()) do
	if v:FindFirstChild("Handle") then
		...
		if v.Name == settings.righthandhat and not handR then
			handR = v
		elseif v.Name == settings.lefthandhat and not handL then
			handL = v
		elseif v.Name == settings.headhat and not head then
...
input.UserCFrameChanged:connect(function(part,move)
	...
	elseif part == Enum.UserCFrame.LeftHand then
		...
		moveHandL.CFrame = cam.CFrame*(CFrame.new(move.p*(cam.HeadScale-1))*move*CFrame.Angles(math.rad(settings.righthandrotoffset.X),math.rad(settings.righthandrotoffset.Y),math.rad(settings.righthandrotoffset.Z)))
	elseif part == Enum.UserCFrame.RightHand then
		--move(handR,cam.CFrame*move)
		moveHandR.CFrame = cam.CFrame*(CFrame.new(move.p*(cam.HeadScale-1))*move*CFrame.Angles(math.rad(settings.righthandrotoffset.X),math.rad(settings.righthandrotoffset.Y),math.rad(settings.righthandrotoffset.Z)))
	end
end)
...
game:GetService("RunService").RenderStepped:connect(function()
	if R1down then
		cam.CFrame = cam.CFrame:Lerp(cam.CoordinateFrame + (moveHandR.CFrame*CFrame.Angles(-math.rad(settings.righthandrotoffset.X),-math.rad(settings.righthandrotoffset.Y),math.rad(180-settings.righthandrotoffset.X))).LookVector * cam.HeadScale/2, 0.5)
	end
end)
...
if settings.forcebubblechat == true then
